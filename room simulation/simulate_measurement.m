function [measurement] = simulate_measurement(laser_pos,objects,params)
    ...
    %% NOTE: Uses GPU. use the _cpu version if this does not work.
        ...
        if strcmp(objects{o,1},'wall') %If this object is a wall
            ...
            for i = linspace(0,1,round(norm(objects{o,3})/params.wall_discr)) %Iterate over surface
                for j = linspace(0,1,round(norm(objects{o,4})/params.wall_discr))
                    ...
                    if  objects{o,2}(3)+i*objects{o,3}(3)+j*objects{o,4}(3)>0 %Check point above floor level
...
%                         else
    ...
    measurement = zeros(ceil((50/params.c)/params.bin_size),size(laser_pos,1));
    
    scene_pixel = gpuArray(scene_pixel);
    scene_pixel_normal = gpuArray(scene_pixel_normal);
    floor_normal = [0,0,1];
    
    fourpi = 4*pi*pi;
   
    for lp = 1:size(laser_pos,1)
       
       sp = (-scene_pixel_angle+light_angle(lp)>=0);
      ...
      % distance_in_time = distance/params.c;
       tbin = gather((distance/(params.c*params.bin_size)));

       dot1 =  max(0,sum(scene_pixel_normal(sp,:).*lps./d1,2));
       dot2 =  max(0,sum(scene_pixel_normal(sp,:).*fovsp./d2,2));
       dot3 = max(0,sum(floor_normal.*-lps./d1,2));
       dot4 = max(0,sum(floor_normal.*-fovsp./d2,2));
       
       intensity = params.laser_intensity.*scene_pixel_area.*gather((dot1.*dot2.*dot3.*dot4)./(fourpi.*d1s.*d2s)); %hemiscpherical spreading
